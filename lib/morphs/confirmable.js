...
module.exports = exports = function Confirmable(schema, options) {
  ...
  schema.methods.generateConfirmationToken = function (done) {
    ...
    confirmable.confirmationToken = getConfirmationToken({
      confirmationTokenExpiryAt,
      email: confirmable.email,
    }, options.confirmable.tokenType);
  ...
  schema.methods.confirm = function (done) {
    ...
    if (!options.confirmable.tokenType || options.confirmable.tokenType !== 'passcode') {
      //If invalid token
 ...
 * @param {*} tokenType 
 */
function getConfirmationToken(payload, tokenType) {
  if (tokenType && tokenType === 'passcode') {
    ...
    //on confirmation token expiry at
    const tokenizer =
      Utils.tokenizer(payload.confirmationTokenExpiryAt.getTime().toString());
