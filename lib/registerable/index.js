...
const { parsePhoneNumber, parsePhoneNumberFromString } = require('libphonenumber-js/mobile');
...
function Registerable(schema, opts = {}) {
    ...
    const fields = {
        ...
        email: {
            ...
            trim: true,
            validate: [{ validator: value => validator.isEmail(value), message: 'Invalid email address {VALUE}' }],
        ...
        phoneNumber: {
            ...
            validate: {
                isAsync: true,
                validator: function (phone, next) {
                    try {
                        const phoneNum = parsePhoneNumber(phone);
                        if (!phoneNum.getType()) {
                            const message = `${phoneNum.number} is not valid mobile phone number`;
                            return next(false, message);
                        }
                        next(true);
                    } catch (error) {
                        const msg = error.message === 'INVALID_COUNTRY'
                            ? 'Invalid phone number country code or it misses plus(+) prefix' :
                            error.message === 'NOT_A_NUMBER' ? 'Invalid mobile phone number' :
                                error.message === 'TOO_LONG' ? 'Too long mobile phone number' :
                                    error.message === 'TOO_SHORT' ? 'Too short mobile phone number' : 'Invalid mobile phone number';
                        next(false, msg);
                    }
                },
        ...
        // control whether to automatical send account confirmation instructions during
        // account registration or not.
        autoSendConfirmationInstructions: {
            type: Boolean,
            default: true
        },
    }
    ...
    mongooseHidden.call(null, schema);
    ...
    */
    ...
    schema.statics.register = function (profile) {
        ...
        return user
            ...
            .then(registerable => {
                if (registerable.confirmationToken && registerable.autoSendConfirmationInstructions) {
