...
describe('Registerable', function () {
    ...
    describe('Register Account', function () {
        ...
        it('should be able to register account without sending confirmation token', function (done) {
            const email = faker.internet.email();

            const $credentials = {
                email: email,
                password: faker.internet.password(),
                autoSendConfirmationInstructions: false
            };
            ...
            User
                ...
                .then(registerable => {
                    expect(registerable.registeredAt).to.not.be.null;
                    expect(registerable.confirmationSentAt).to.be.undefined;
                    done();
                });
        });
    });
